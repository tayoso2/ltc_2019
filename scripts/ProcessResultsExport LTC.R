...
setwd("C:/Users/TOsosanya/Desktop/LTC/Output Data")
...
# # Dataflow R.Execute(1.0)
# fold.wrk       <- getwd()
#
# # default root input and output folders
# fold.in        <- file.path(fold.wrk, 'Input')
# fold.out       <- file.path(fold.wrk, 'Output')
#
# # input folder structure
# getwd()  <- file.path(fold.in, 'AssetResultExport')
#
# # output folder structure
# fold.out.dashboard <- file.path(fold.out, 'Dashboard')
# if(!dir.exists(fold.out.dashboard)){
#   dir.create(fold.out.dashboard)
# }
...
MeltSingleMeasure <-
  function(measure, result, attributes, timesteps) {
    ...
    x$Timestep <- str_replace(x$Measure, measure, "")
    # Remove measure name
    x$Measure <- NULL
...
MakeLong <-
  function(result,
    ...
    if (is.character(result)) {
      result <- read.csv(result, check.names = FALSE, fill = FALSE)
    }
...
MultipleMakeLong <-
  function(files,
           scenarios = NULL,
           measures = c("Length","Age","Age1","Age2","Age3", "PCIRiskCost", "AADT", "CumYE4","PCI","IRI", "ACA","NPT","ARV","SFC","TD","DMI","SNP", "SNC", "RDM","RDS","Average IRI",
                        "Average PCI","ReplacementCount","RejuvenationCost", "FogSealCost","ResurfacingCost", "OverlayCost","MillandReplaceCost",
                        "SealThermalCracksCost","SelectiveMillingandPatchCost","Health_Score","Customer_Cost_Risk", "Tunnel_Cost_Risk", "Service_Quality_Risk",
                        "Environmental_Risk","Safety_Risk","Replacement_Cost","PCIRiskCost","NP_Total_Risk_Cost"),
           attributes = NULL,
           timesteps = NULL) {
...
### Examples ###
asset.attributes <- read.csv("P_NP Maintain Asset Condition Assets.csv" , check.names = FALSE)
asset.attributes <- asset.attributes[, 1:24]

# Convert several files to long in one operation
long.result <-
  MultipleMakeLong(
    files = c("P_NP NI Assets.csv","P_NP Maintain Asset Condition Assets.csv","P_NP Maintain Risk Assets.csv"),
    scenarios = c("No Investment","Maintain Asset Condition","Maintain Risk"),
    attributes = c("AssetID")
  )


#semi.long.results <- MakeSemiLong(long.results)
dir.create("C:/Users/TOsosanya/Desktop/LTC/Process Result")
write.csv(long.result, "C:/Users/TOsosanya/Desktop/LTC/Process Result/long_results.csv", na = "")
write.csv(asset.attributes, "C:/Users/TOsosanya/Desktop/LTC/Process Result/attributes.csv", na = "")


# get likelihood and consequence
long.result2 <- long.result %>% dplyr::select(Scenario, AssetID, Timestep) %>% unique()
long.result2$Likelihood <- signif(runif(nrow(long.result2),1,6), 1)
long.result2$Consequence <- signif(runif(nrow(long.result2),1,6), 1)

# write the output file
write.csv(long.result2, "C:/Users/TOsosanya/Desktop/LTC/Process Result/LandC.csv", na = "")
...
pcifactor <- read.csv("C:/Users/TOsosanya/Desktop/LTC/Process Result/PCIFactor Lookup.csv")
long_results <- read.csv("C:/Users/TOsosanya/Desktop/LTC/Process Result/long_results.csv")
long_results2 <- long_results %>% left_join(pcifactor, by = "AssetID") %>%
  mutate(Value = ifelse(Measure == "PCI" & Value < 90 & Timestep > 0 & !is.na(PCIFactor),Value/PCIFactor,Value)) %>%
  dplyr::select(-c(PCIFactor,X))
summary(long_results2)
long_results2 %>% filter(Measure == "PCI", Value < 90, Timestep > 0)

# write the output file
write.csv(long_results2, "C:/Users/TOsosanya/Desktop/LTC/Process Result/long_results2.csv", na = "")
